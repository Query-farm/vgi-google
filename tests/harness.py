"""In-process VGI table-function invocation for the google test suite.

Runs a table function through the real bind -> init -> process lifecycle
(adapted from the vgi-embed / vgi-scholar harness), driving ``process()``
repeatedly until it signals finish. State is the live ``_ScanState`` object, so
this exercises the same pageToken-advancement logic the framework uses on every
tick — and the cursor round-trip across batches is genuinely tested.
"""
...
_MAX_TICKS = 100_000  # guard against an accidental infinite loop in a test
...
def invoke_table_function(
    ...
    *,
    ...
    while not out.finished:
        ...
        ticks += 1
        if ticks > max_ticks:
            raise AssertionError(f"{func_cls.Meta.name} did not finish within {max_ticks} ticks")
